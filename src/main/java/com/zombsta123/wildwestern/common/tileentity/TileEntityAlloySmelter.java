...
public class TileEntityAlloySmelter extends TileEntity implements IInventory, ITickable {
	...
	public void setInventorySlotContents(int index, ItemStack stack) {
		...
		boolean flag = !stack.isEmpty() && stack.isItemEqual(itemstack)
				&& ItemStack.areItemStackShareTagsEqual(stack, itemstack);
	...
	public void update() {
		...
		if (!this.world.isRemote) {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				BlockAlloySmelter.setState(this.isBurning(), this.world, this.pos);
			}
		}
		if (flag1)
			this.markDirty();
	...
	private boolean canSmelt() {
		if (((ItemStack) this.inventory.get(0)).isEmpty() || ((ItemStack) this.inventory.get(1)).isEmpty())
			...
			ItemStack result = AlloySmelterRecipes.getInstance().getAlloyResult((ItemStack) this.inventory.get(0),
					(ItemStack) this.inventory.get(1));
			if (result.isEmpty())
				return false;
			else {
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			...
			ItemStack result = AlloySmelterRecipes.getInstance().getAlloyResult(input1, input2);
			ItemStack output = (ItemStack) this.inventory.get(3);
	...
	public String getGuiID() {
		return "wildwestern:alloy_smelter";
	}
