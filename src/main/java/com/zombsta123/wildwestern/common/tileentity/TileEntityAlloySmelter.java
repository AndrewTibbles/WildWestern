package com.zombsta123.wildwestern.common.tileentity;

import com.zombsta123.wildwestern.common.blocks.BlockAlloySmelter;
import com.zombsta123.wildwestern.core.recipes.AlloySmelterRecipes;
...
public class TileEntityAlloySmelter extends TileEntity implements ITickable {
	...
	public static boolean isBurning(TileEntityAlloySmelter tileentityas) {
		return tileentityas.getField(0) > 0;
	}

	public void update() {
		if (this.isBurning()) {
			--this.burnTime;
			BlockAlloySmelter.setState(true, world, pos);
		}
		...
		if (this.isBurning() && this.canSmelt() && cookTime > 0) {
			...
			if (this.canSmelt() && this.isBurning()) {
				ItemStack output = AlloySmelterRecipes.getInstance().getAlloyResult(inputs[0], inputs[1]);
				if (!output.isEmpty()) {
					smelting = output;
					cookTime++;
					inputs[0].shrink(1);
	...
	private boolean canSmelt() {
		if (((ItemStack) this.handler.getStackInSlot(0)).isEmpty()
			...
			ItemStack result = AlloySmelterRecipes.getInstance().getAlloyResult(
